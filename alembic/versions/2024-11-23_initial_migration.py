...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_tariffs_relevance_date"), "tariffs", ["relevance_date"], unique=False
    )
    op.create_table(
        "material_rates",
        sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
...
def downgrade() -> None:
    op.drop_index(
        "ix_material_rates_tariff_id_material_type", table_name="material_rates"
    )
